...
import { useMockSession } from './features/tracker/hooks/useMockSession';
import type { TrackerRole } from './features/tracker/types/user';
import { AppProviders } from './app/providers/AppProviders';
import { allRoutes, appRoutes, publicRoutes, routes } from './shared/constants/routes';
...
export default function App() {
  ...
  const appRole = getRoleFromAppPath(pathname) ?? auth.session?.role ?? 'student';
    ...
    switch (appRole) {
      case 'teacher':
        return {
          ...
          subtitle: 'Созданные задания, статусы по ученикам, очередь на проверку и более чистая рабочая зона без лишнего шума.',
        };
      case 'parent':
        return {
          title: 'Кабинет родителя',
          subtitle: 'Прогресс ребенка, сигналы по рискам, достижения и понятная картина без перегруженного журнала.',
        };
      default:
        return {
          title: 'Кабинет ученика',
          subtitle: 'Сегодняшние задачи, ближайшие дедлайны, просрочки и личный прогресс в одном рабочем ритме.',
        };
    }
  }, [appRole]);
    ...
    return (
      <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.12),transparent_22%),radial-gradient(circle_at_top_right,rgba(245,158,11,0.12),transparent_18%),linear-gradient(180deg,#f8fafc_0%,#eef2ff_46%,#f8fafc_100%)] px-4 py-5 text-slate-900 md:px-6">
        <div className="mx-auto max-w-[1500px]">
          ...
          >
            {pathname === routes.studentApp && <StudentDashboardPage tasksApi={tasksApi} />}
  ...
  if (isAppPath(pathname)) {
    return <AppProviders>{renderAppPage()}</AppProviders>;
  }

  return <AppProviders>{renderPublicPage()}</AppProviders>;
}
